#include "linkedlist2.h"

int main(){

    LinkedList queue1,queue2;
    int choice,value,position,queueChoice;

    do{

        cout<<"1 Insert at End"<<endl;
        cout<<"2 Insert at Start"<<endl;
        cout<<"3 Insert at Position"<<endl;
        cout<<"4 Delete from Start"<<endl;
        cout<<"5 Delete from End"<<endl;
        cout<<"6 Delete at Position"<<endl;
        cout<<"7 Search"<<endl;
        cout<<"8 Reverse"<<endl;
        cout<<"9 Count"<<endl;
        cout<<"10 Display"<<endl;
        cout<<"11 Largest"<<endl;
        cout<<"12 Smallest"<<endl;
        cout<<"13 Sum"<<endl;
        cout<<"14 Average"<<endl;
        cout<<"15 Split"<<endl;
        cout<<"16 Concatenate"<<endl;
        cout<<"17 Third Largest"<<endl;
        cout<<"18 Check Empty"<<endl;
        cout<<"19 Peek"<<endl;
        cout<<"20 Display Both"<<endl;
        cout<<"0 Exit"<<endl;

        cin>>choice;

        if(choice>=1 && choice<=14 || choice==17 || choice==18 || choice==19){
            cout<<"Select Queue (1 or 2): ";
            cin>>queueChoice;
        }

        LinkedList* selectedQueue=NULL;

        if(queueChoice==1)
            selectedQueue=&queue1;
        else if(queueChoice==2)
            selectedQueue=&queue2;

        switch(choice){

        case 1:
            cin>>value;
            selectedQueue->insertAtEnd(value);
            break;

        case 2:
            cin>>value;
            selectedQueue->insertAtStart(value);
            break;

        case 3:
            cin>>value>>position;
            selectedQueue->insertAtPosition(value,position);
            break;

        case 4:
            selectedQueue->deleteAtStart();
            break;

        case 5:
            selectedQueue->deleteFromEnd();
            break;

        case 6:
            cin>>position;
            selectedQueue->deleteAtPosition(position);
            break;

        case 7:
            cin>>value;
            if(selectedQueue->search(value))
                cout<<"Found"<<endl;
            else
                cout<<"Not Found"<<endl;
            break;

        case 8:
            selectedQueue->reverse();
            break;

        case 9:
            cout<<selectedQueue->countNodes()<<endl;
            break;

        case 10:
            selectedQueue->display();
            break;

        case 11:
            cout<<selectedQueue->findLargest()<<endl;
            break;

        case 12:
            cout<<selectedQueue->findSmallest()<<endl;
            break;

        case 13:
            cout<<selectedQueue->getSum()<<endl;
            break;

        case 14:
            cout<<selectedQueue->getAverage()<<endl;
            break;

        case 15:{
            int targetQueue;
            cin>>queueChoice>>position>>targetQueue;

            if(queueChoice==1 && targetQueue==2)
                queue1.split(position,queue2);
            else if(queueChoice==2 && targetQueue==1)
                queue2.split(position,queue1);

            break;
        }

        case 16:{
            int first,second;
            cin>>first>>second;

            if(first==1 && second==2)
                queue1.concatenate(queue2);
            else if(first==2 && second==1)
                queue2.concatenate(queue1);

            break;
        }

        case 17:
            cout<<selectedQueue->findThirdLargest()<<endl;
            break;

        case 18:
            if(selectedQueue->countNodes()==0)
                cout<<"Empty"<<endl;
            else
                cout<<"Not Empty"<<endl;
            break;

        case 19:
            cout<<selectedQueue->peek()<<endl;
            break;

        case 20:
            queue1.display();
            queue2.display();
            break;

        }

    }while(choice!=0);

    return 0;
}